...
let projects = [
  {
    image: 'textures/kms.png',
    url: 'https://github.com/IftekherFahim27/Gym-management-system-ms-sql-server08',
  },
  {
    image: 'textures/Movcorn.jpg',
    url: 'https://github.com/IftekherFahim27/MovCorn-webdev-project-3.1',
  },
  {
    image: 'textures/pathtojannah.jpg',
    url: 'https://github.com/IftekherFahim27/PathToJannah',
  },
  {
    image: 'textures/surprisepop.bmp',
    url: 'https://github.com/IftekherFahim27/Surprise-Pop',
  },
];
...
let certificate = [
  {
    title: 'CGPA',
    image: 'textures/cgpa_logo.png',
    url: 'https://drive.google.com/file/d/16WnGVXAeudlzzKLRJja127Cm7EzbbIFm/view?usp=drive_link',
  },
  {
    title: 'HSC Certificate',
    image: 'textures/hsc_logo.png',
    url: 'https://drive.google.com/file/d/1G_EhEBNTLi-PaOFTjszeJor3zAbbfZOO/view?usp=drive_link',
  },

  {
    title: 'SSC Certificate',
    image: 'textures/ssc_logo.png',
    url: 'https://drive.google.com/file/d/13t18zL8gc41ABJyBoRjLSPeNNndLyGUY/view?usp=drive_link',
  },
...
gltfLoader.load(
  ...
  function (room) {
    ...
    video.src = 'textures/Interstellar.mp4';
    video.muted = true;
    ...
    video3.src = 'textures/keyboard.mp4';
    video3.muted = true;
    ...
    videoTexture.encoding = THREE.sRGBEncoding;
     ...
     // create keyboard video texture
     const videoTexture3 = new THREE.VideoTexture(video3);
     videoTexture3.minFilter = THREE.NearestFilter;
     videoTexture3.magFilter = THREE.NearestFilter;
     videoTexture3.generateMipmaps = false;
     videoTexture3.encoding = THREE.sRGBEncoding;

      // create smoke  video texture
      const videoTexture4 = new THREE.VideoTexture(video4);
      videoTexture4.minFilter = THREE.NearestFilter;
      videoTexture4.magFilter = THREE.NearestFilter;
    ...
    room.scene.children.forEach((child) => {
      ...
      if (child.name === 'Table') {
        // Change the color of the table material
        const tableMaterial = new THREE.MeshStandardMaterial({
          color: 0xA1662F, // Change this color to the desired color in hexadecimal format
          roughness: 0.8,
          metalness: 0.2,
        });
      ...
      if (child.name === 'Coffe') {
        // Change the color of the table material
        const tableMaterial = new THREE.MeshStandardMaterial({
          color: 0xb14a30, // Change this color to the desired color in hexadecimal format
          roughness: 0.8,
          metalness: 0.2,
        });
        child.material = tableMaterial;
      ...
      if (child.name === 'Glass') {
        // Change the color of the table material
        const tableMaterial = new THREE.MeshStandardMaterial({
          color: 0x0b0b0b, // Change this color to the desired color in hexadecimal format
          roughness: 0.5,
          metalness: 0.2,
        });
        child.material = tableMaterial;
      ...
      if (child.name === 'SwitchBoard') {
        // Change the color of the table material
        const tableMaterial = new THREE.MeshStandardMaterial({
          color: 0x2E4619, // Change this color to the desired color in hexadecimal format
          roughness: 0.8,
          metalness: 0.2,
        });
        child.material = tableMaterial;
      ...
      if (child.name === 'Book') {
        ...
        const newIconTexture = new THREE.TextureLoader().load('textures/fahim.png');
        newIconTexture.flipY = false; // Adjust this if needed

        // Replace the existing texture of child.children[0] with the new icon texture
        child.children[0].material.map = newIconTexture;
...
// ADD LIGHT
const ambientLight = new THREE.AmbientLight(0x9999ff, 0.6);
scene.add(ambientLight);
const roomLight = new THREE.PointLight(0x9999ff, 2.5, 10);
...
const fanLight2 = new THREE.PointLight(0x7FF4F9, 30, 0.12);
const fanLight3 = new THREE.PointLight(0x7FF4F9, 30, 0.2);
const fanLight4 = new THREE.PointLight(0x7FF4F9, 30, 0.2);
const fanLight5 = new THREE.PointLight(0x7FF4F9, 30, 0.4);
...
fanLight5.position.set(0.21, 0.6, -0.4);
scene.add(fanLight1);
scene.add(fanLight2);
scene.add(fanLight3);
scene.add(fanLight4);
...
const pointLight1 = new THREE.PointLight(0x7FF4F9, 0, 1.1);
const pointLight2 = new THREE.PointLight(0x7FF4F9, 0, 1.1);
const pointLight3 = new THREE.PointLight(0x7FF4F9, 0, 1.1);
const pointLight4 = new THREE.PointLight(0x7FF4F9, 0, 1.1);
...
function loadIntroText() {
  ...
  loader.load('fonts/droid_serif_bold.typeface.json', function (font) {
    ...
    const titleGeo = new TextGeometry('Iftekher Fahim', {
      font: font,
      size: 0.08,
      height: 0.01,
    });
  ...
  loader.load('fonts/unione.json', function (font) {
    ...
    const subTitleGeo = new TextGeometry(
      'Student / Developer / Class Representative',
      {
        font: font,
        size: 0.018,
        height: 0,
...
function logoListener() {
  document.getElementById('aust_logo').addEventListener('click', function (e) {
    e.preventDefault();
    resetCamera();
  });
}
...
function certificateMenuListener() {
  // create project planes with textures
  certificate.forEach((project, i) => {
    const colIndex = i % 3 === 0 ? 0 : 1;
    const rowIndex = Math.floor(i / 3);
    ...
    projectPlane.scale.set(0, 0, 0);
    // mesh & y vars needed for animation
    certificate[i].mesh = projectPlane;
    certificate[i].y = 1 - rowIndex * 0.5;
    scene.add(projectPlane);
    
  });

  document
    .getElementById('edu-menu')
    .addEventListener('click', function (e) {
      e.preventDefault();
      disableOrbitControls();
      resetBookCover();
...
function init3DWorldClickListeners() {
  ...
  window.addEventListener('click', function (e) {
    ...
    const certificateBtn = document.getElementById('edu-menu');
    if (
      e.target === closeBtn ||
      closeBtn.contains(e.target) ||
      e.target === projectsBtn ||
      projectsBtn.contains(e.target) ||
      e.target === certificateBtn ||
      certificateBtn.contains(e.target)
    ) {
      return false;
    }
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  bioMenu.addEventListener("click", function() {
    window.open("190104027.html", "_blank");
  });
});
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  workMenu.addEventListener("click", function() {
    const websiteURL = 'https://www.facebook.com/grameenwatertechnology'; // Replace with the URL of the website you want to open
...
document.addEventListener("DOMContentLoaded", function() {
  const workMenu = document.getElementById("aust_logo");

  workMenu.addEventListener("click", function() {
    const websiteURL = 'https://aust.edu/'; // Replace with the URL of the website you want to open
